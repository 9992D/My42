...
{
    if( type == PIPE || type == OR || type == AMPERSAND 
    || type == AND || type == REDIR_IN || type == HEREDOC 
...
{
    if( type == T_ECHO || type == T_CD || type == T_PWD 
    || type == T_EXPORT || type == T_UNSET || type == T_ENV 
    || type == T_EXIT)
        return 1;
    return 0;
}
