...
void	print_command_structure(t_command *cmd_list)
{
	while (cmd_list)
	{
        ...
        if (cmd_list->cmd == NOT_BUILTIN)
		    printf("(cmd)[NOT_BUILTIN] -> %s (char *not_builtin pas une target)\n", cmd_list->not_builtin);
        else
		...
		{
			printf("    (args)[%s] -> %s\n",
				get_type_name(cmd_list->args[i].type), cmd_list->args[i].str);
		}
		/* 3) Les redirections */
...
int	main(void)
{
	...
	while (1)
	{
		...
		char_list = build_token_list(line);
		if (!char_list)
		{
		...
		token_list = convert_to_tokens(char_list);
		if (!token_list)
		{
